...
router.get("/current", requireAuth, async (req, res) => {
  ...
  return res.json({
    Reviews: reviewArray.map((aReview) => ({
      ...aReview,
      createdAt: aReview.createdAt
        .toISOString()
        .replace("T", " ")
        .split(".")[0],
      updatedAt: aReview.updatedAt
        .toISOString()
        .replace("T", " ")
        .split(".")[0],
    })),
...
router.post("/:reviewId/images", requireAuth, async (req, res) => {
  ...
  if (req.user.id === review.userId) {
    ...
    return res.json({
      message:
        "Forbidden: Review must belong to the current user in order to add an image for a review",
    });
  }
});
...
router.delete("/:reviewId", requireAuth, async (req, res) => {
  ...
  if (req.user.id !== currentReview.userId) {
    ...
    res.json({
      message:
        "Forbidden: Current user is not authorized to delete this review",
    });
  }
