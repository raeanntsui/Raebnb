  ...
  if (spot.ownerId !== req.user.id) {
    ...
    return res.json({
      Bookings: allBookings.map((aBooking) => ({
        ...
        startDate: aBooking.startDate.toISOString().split("T")[0],
        endDate: aBooking.endDate.toISOString().split("T")[0],
        createdAt: aBooking.createdAt
          .toISOString()
          .replace("T", " ")
          .split(".")[0],
        updatedAt: aBooking.updatedAt
          .toISOString()
          .replace("T", " ")
          .split(".")[0],
      })),
...
router.get("/current", async (req, res) => {
  ...
  spotArray.forEach((spot) => {
    ...
    if (spot.previewImage === undefined)
      spot.previewImage = "No URL for Spot found.";
    delete spot.SpotImages;
  });

  res.status(200);
  return res.json({
    Spots: spotArray.map((aSpot) => ({
      ...aSpot,
      createdAt: aSpot.createdAt.toISOString().replace("T", " ").split(".")[0],
      updatedAt: aSpot.updatedAt.toISOString().replace("T", " ").split(".")[0],
    })),
  });
});
...
router.get("/", queryFilter, async (req, res) => {
  ...
  spotsList.forEach((oneSpot) => {
    ...
    if (!oneSpot.avgRating)
      oneSpot.avgRating = "No average rating for Spot found.";
  ...
  spotsList.forEach((oneSpot) => {
    ...
    if (!oneSpot.previewImage)
      oneSpot.previewImage = "No URL for preview image for Spot found.";
    delete oneSpot.SpotImages;
  });

  return res.json({
    Spots: spotsList.map((aSpot) => ({
      ...aSpot,
      createdAt: aSpot.createdAt.toISOString().replace("T", " ").split(".")[0],
      updatedAt: aSpot.updatedAt.toISOString().replace("T", " ").split(".")[0],
    })),
...
router.get("/:spotId", async (req, res) => {
  ...
  // set current total star rating to 0
  let totalStars = 0;
  ...
  return res.json({
    ...
    price: spot.price,
    createdAt: spot.createdAt.toISOString().replace("T", " ").split(".")[0],
    updatedAt: spot.updatedAt.toISOString().replace("T", " ").split(".")[0],
...
router.post("/:spotId/bookings", requireAuth, async (req, res) => {
  ...
  if (spot.ownerId === req.user.id) {
    ...
    return res.json({
      message:
        "Forbidden: Spot belongs to the current user. Cannot create a booking.",
    });
  }
  ...
  async (req, res) => {
    ...
    if (existingReview) {
      ...
      return res.json({
        ...
        createdAt: newReviewForSpot.createdAt
          .toISOString()
          .replace("T", " ")
          .split(".")[0],
        updatedAt: newReviewForSpot.updatedAt
          .toISOString()
          .replace("T", " ")
          .split(".")[0],
      });
...
router.post("/", requireAuth, validateSpot, async (req, res) => {
  ...
  return res.json({
    ...newSpot.toJSON(),
    createdAt: newSpot.createdAt.toISOString().replace("T", " ").split(".")[0],
    updatedAt: newSpot.updatedAt.toISOString().replace("T", " ").split(".")[0],
  });
});
...
router.delete("/:spotId", requireAuth, async (req, res) => {
  ...
  if (spot) {
    ...
    if (spot.ownerId === req.user.id) {
      ...
      return res.json({
        message:
          "Forbidden: Current user is not authorized to delete this spot",
      });
    }
  } else if (!spot) {
    res.status(404);
